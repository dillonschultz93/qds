...
path. It comes from the token's source file, which is why generating these names needs a custom
Style Dictionary transform rather than the built-in `name/kebab`.
...
shipping. That matters most for tokens arriving *from* Figma: Token Studio syncs `tokens/`
...
`tokens/` is authored in Token Studio's multi-file layout, and the plugin syncs it directly from Git
— bidirectionally. There is no export step to Figma.

```
tokens/
├── $metadata.json           tokenSetOrder
├── $themes.json             Light and Dark, one "mode" group
├── primitive/               color, typography, spacing, border, shadow, animation
├── semantic/
│   ├── base.json            mode-invariant: typography, radius, border, animation
│   ├── light.json           colors and elevation
│   └── dark.json            colors and elevation
└── component/
    └── button.json
```

Mode-invariant semantics live in `semantic/base.json` rather than being duplicated across
`light.json` and `dark.json`. Typography and radius do not change between modes, and duplicating
them would mean every edit had to be made twice — with the theme-completeness check enforcing the
duplication rather than preventing the drift.

The mode is expressed by the **CSS selector**, never by the token name. `semantic/light.json` and
`semantic/dark.json` produce identical variable names, emitted under `:root` and
`[data-theme="dark"]` respectively.
